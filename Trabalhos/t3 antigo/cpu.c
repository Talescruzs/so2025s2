...
// uma CPU tem estado, memória (via MMU), controlador de ES
struct cpu_t {
  // registradores
  int PC;
  int A;
...
{
  // quem criou mem física, mmu e e/s que destrua!
  free(self);
}
...
// lê um valor da memória (virtual)
// usa a MMU; se houver erro, coloca em self->erro e self->complemento
static bool pega_mem(cpu_t *self, int endereco, int *pval)
{
  // em cpu, não fazemos checagem de região; a MMU tratará tradução/privilegio
...
// escreve um valor na memória (virtual)
...
// (as implementações das instruções permanecem as mesmas, somente usando pega_mem/poe_mem)

static void op_NOP(cpu_t *self) { self->PC += 1; }
static void op_PARA(cpu_t *self) { self->erro = ERR_CPU_PARADA; }
static void op_CARGI(cpu_t *self) {
  int A1;
  if (pega_A1(self, &A1)) { self->A = A1; self->PC += 2; }
}
static void op_CARGM(cpu_t *self) {
  int A1, mA1;
  if (pega_A1(self, &A1) && pega_mem(self, A1, &mA1)) { self->A = mA1; self->PC += 2; }
}
static void op_CARGX(cpu_t *self) {
  int A1, mA1mX;
  int X = self->X;
  if (pega_A1(self, &A1) && pega_mem(self, A1 + X, &mA1mX)) { self->A = mA1mX; self->PC += 2; }
}
static void op_ARMM(cpu_t *self) {
  int A1;
  if (pega_A1(self, &A1) && poe_mem(self, A1, self->A)) { self->PC += 2; }
}
static void op_ARMX(cpu_t *self) {
  int A1;
  int X = self->X;
  if (pega_A1(self, &A1) && poe_mem(self, A1 + X, self->A)) { self->PC += 2; }
}
static void op_TRAX(cpu_t *self) { int A = self->A; int X = self->X; self->X = A; self->A = X; self->PC += 1; }
static void op_CPXA(cpu_t *self) { self->A = self->X; self->PC += 1; }
static void op_INCX(cpu_t *self) { self->X += 1; self->PC += 1; }
static void op_SOMA(cpu_t *self) { int A1, mA1; if (pega_A1(self, &A1) && pega_mem(self, A1, &mA1)) { self->A += mA1; self->PC += 2; } }
static void op_SUB(cpu_t *self) { int A1, mA1; if (pega_A1(self, &A1) && pega_mem(self, A1, &mA1)) { self->A -= mA1; self->PC += 2; } }
static void op_MULT(cpu_t *self) { int A1, mA1; if (pega_A1(self, &A1) && pega_mem(self, A1, &mA1)) { self->A *= mA1; self->PC += 2; } }
static void op_DIV(cpu_t *self) { int A1, mA1; if (pega_A1(self, &A1) && pega_mem(self, A1, &mA1)) { self->A /= mA1; self->PC += 2; } }
static void op_RESTO(cpu_t *self) { int A1, mA1; if (pega_A1(self, &A1) && pega_mem(self, A1, &mA1)) { self->A %= mA1; self->PC += 2; } }
static void op_NEG(cpu_t *self) { self->A = -self->A; self->PC += 1; }
static void op_DESV(cpu_t *self) { int A1; if (pega_A1(self, &A1)) { self->PC = A1; } }
static void op_DESVZ(cpu_t *self) { if (self->A == 0) { op_DESV(self); } else { self->PC += 2; } }
static void op_DESVNZ(cpu_t *self) { if (self->A != 0) { op_DESV(self); } else { self->PC += 2; } }
static void op_DESVN(cpu_t *self) { if (self->A < 0) { op_DESV(self); } else { self->PC += 2; } }
static void op_DESVP(cpu_t *self) { if (self->A > 0) { op_DESV(self); } else { self->PC += 2; } }
static void op_CHAMA(cpu_t *self) { int A1; if (pega_A1(self, &A1) && poe_mem(self, A1, self->PC + 2)) { self->PC = A1 + 1; } }
static void op_RET(cpu_t *self) { int A1, mA1; if (pega_A1(self, &A1) && pega_mem(self, A1, &mA1)) { self->PC = mA1; } }

static void op_LE(cpu_t *self) { int A1, dado; if (pega_A1(self, &A1) && pega_es(self, A1, &dado)) { self->A = dado; self->PC += 2; } }
static void op_ESCR(cpu_t *self) { int A1; if (pega_A1(self, &A1) && poe_es(self, A1, self->A)) { self->PC += 2; } }

// RETI precisa de cpu_desinterrompe, que tá lá embaixo
static void cpu_desinterrompe(cpu_t *self);

static void op_RETI(cpu_t *self) { cpu_desinterrompe(self); }

static void op_CHAMAC(cpu_t *self) {
  if (self->func_chamaC == NULL) { self->erro = ERR_OP_INV; return; }
  self->A = self->func_chamaC(self->arg_chamaC, self->A);
  self->PC += 1;
}

static void op_CHAMAS(cpu_t *self) {
  self->PC += 1;
...
bool cpu_interrompe(cpu_t *self, irq_t irq)
{
  ...
  // Copia o estado da CPU para variáveis locais
  ...
  // põe a CPU em modo supervisor
  self->modo = supervisor;

  // salva todo o estado interno da CPU usando a MMU (em modo supervisor; a MMU repassa como acesso físico)
  mmu_escreve(self->mmu, CPU_END_PC,          PC, self->modo);
  mmu_escreve(self->mmu, CPU_END_A,           A, self->modo);
  mmu_escreve(self->mmu, CPU_END_erro,        erro, self->modo);
  mmu_escreve(self->mmu, CPU_END_complemento, complemento, self->modo);
...
{
  int PC, A, erro, complemento;
  mmu_le(self->mmu, CPU_END_PC,          &PC, self->modo);
  mmu_le(self->mmu, CPU_END_A,           &A, self->modo);
  mmu_le(self->mmu, CPU_END_erro,        &erro, self->modo);
  mmu_le(self->mmu, CPU_END_complemento, &complemento, self->modo);
